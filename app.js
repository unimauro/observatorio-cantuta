...
const GRANATE = '#c2185b', GRANATE2 = '#9e1852', ORO = '#d99a2b', TEAL = '#0e7c86';
...
const ink = () => isDark() ? '#c9a4b6' : '#7a6069';
...
async function boot() {
  DATA.presu = await load('presupuesto-cantuta.json');
  DATA.biblio = await load('bibliometria.json');
  DATA.prov = await load('proveedores-cantuta.json');
  DATA.plan = await load('planilla-cantuta.json');
  render();
}
...
function renderGasto() {
  ...
  if (note) note.innerHTML = `Ejercicio ${d.anio} (cerrado). ` +
    ...
    `Fuente: MEF/SIAF, pliego 528 (La Cantuta).`;
}
...
function renderProv() {
  ...
  if (!V?.proveedores?.length) {
    w.innerHTML = `<div class="soon"><h3>🔎 En construcción</h3><p style="margin:0;color:var(--muted)">Descargando y cruzando las contrataciones de La Cantuta (OECE/CONOSCE) para listar cada proveedor, su RUC, el monto y sus dueños/representantes.</p></div>`;
    return;
  }
  const t = V.totales || {};
  // KPIs
  ...
  w.innerHTML = `<div class="card">
    <h3>Proveedores de La Cantuta · ${fmtN(V.proveedores.length)}</h3>
...
function renderInv() {
  ...
  const peers = [['uni', 'La Cantuta'], ['sanmarcos', 'San Marcos'], ['pucp', 'PUCP'], ['cayetano', 'Cayetano'], ['unalm', 'La Molina'], ['uni_ing', 'UNI']]
    .map(([k, n]) => DATA.biblio[k] ? { n, v: DATA.biblio[k].works } : null).filter(Boolean).sort((a, b) => b.v - a.v);
  new Chart(cPares, {
    type: 'bar', data: { labels: peers.map(x => x.n), datasets: [{ label: 'Publicaciones', data: peers.map(x => x.v), backgroundColor: peers.map(x => x.n === 'La Cantuta' ? GRANATE : ORO) }] },
    options: opts({ indexAxis: 'y', plugins: { legend: { display: false } } })
  });
  document.getElementById('invNote').textContent = `La Cantuta: ${fmtN(b.works)} publicaciones, ${fmtN(b.cited)} citas, h-index ${b.h_index} (OpenAlex, ${DATA.biblio._meta?.extraido || '2026'}). Como universidad formadora de docentes, su producción científica es modesta frente a las universidades de investigación; interpretar el volumen con ese contexto.`;
}
...
function renderPlan() {
  ...
  if (!P?.personas?.length) {
    w.innerHTML = `<div class="soon"><h3>🔎 En construcción</h3><p style="margin:0;color:var(--muted)">Cruzando la planilla de La Cantuta (docentes, funcionarios y personal, con sus remuneraciones) desde datos públicos del Estado (AIRHSP / portal de transparencia). Este portal <strong>solo muestra personal — nunca nombres de estudiantes</strong>.</p></div>`;
    return;
  }
...
function buildCtx() {
  ...
  let c = 'Eres el asistente del Observatorio La Cantuta, portal ciudadano de transparencia de la Universidad Nacional de Educación Enrique Guzmán y Valle (UNE La Cantuta, pliego 528), con datos públicos (MEF, OECE, OpenAlex). Responde corto, en español, solo sobre La Cantuta y sus datos. No inventes cifras.';
